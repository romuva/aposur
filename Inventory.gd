...
func _input(event):
	if event is InputEventMouseMotion:
		if holdingItem != null && holdingItem.picked:
			holdingItem.rect_global_position = Vector2(get_parent().get_global_rect().position.x + event.position.x - get_parent().get_global_rect().position.x - 150, self.get_global_rect().position.y + event.position.y - get_parent().get_global_rect().position.y - 120)
