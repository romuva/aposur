...
remotesync var MAX_WEIGHT_COUNT = 20

remotesync var waterCount = 10
remotesync var foodCount = 100
...
func _on_Area2DTown_area_entered(area):
	area.town_entered()
	
	if(waterCount + area.waterCount <= area.MAX_WATER_COUNT):
		area.waterCount += waterCount
		waterCount = 0
	else:
		waterCount = waterCount - (area.MAX_WATER_COUNT - area.waterCount)
		area.waterCount = area.MAX_WATER_COUNT
		
#	if(foodCount + area.foodCount <= area.MAX_FOOD_COUNT):
#		area.foodCount += foodCount
#		foodCount = 0
#	else:
#		foodCount = foodCount - (area.MAX_FOOD_COUNT - area.foodCount)
#		area.foodCount = area.MAX_FOOD_COUNT
		
	if(ammoCount + area.ammoCount <= area.MAX_AMMO_COUNT):
		area.ammoCount += ammoCount
		ammoCount = 0
	else:
		ammoCount = ammoCount - (area.MAX_AMMO_COUNT - area.ammoCount)
		area.ammoCount = area.MAX_AMMO_COUNT
	
	area._update_item_labels()
	_update_town_labels()


func _on_Area2DTown_area_exited(area):
	area.town_exited()
